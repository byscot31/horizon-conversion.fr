...
/** hub -> leaf : pages locales “phares” */
const HUB_LEAF_LINKS = {
    "seo-local": [
        "/seo-local/chantilly",
        "/seo-local/senlis",
        "/seo-local/creil",
...
function patchPagesArray(pages) {
    for (const p of pages) {
        ...
        // keep legal pages as-is (they have faq: [])
        ...
        // hub -> leaf on service pages
        if (p.type === "service" && p.service && HUB_LEAF_LINKS[p.service]) {
            const baseLinks = Array.isArray(p.linksOut) ? p.linksOut : [];
            p.linksOut = uniq([...baseLinks, ...HUB_LEAF_LINKS[p.service]]);
        }
    }
}
...
function patchComputed() {
    ...
    // articles: on garde faq vide (pas besoin)
...
console.log("✅ seo-mapping.json patché : faq -> faqRefs + hub->leaf linksOut");